...
class NppaymentsController extends AppController
{
    ...
    public function index()
    {
        ...
        $nppayments = $this->paginate($this->Nppayments->find()->where(['company_id'=>$st_company_id])->contain(['NppaymentRows'=>function($q){
            $NppaymentRow = $this->Nppayments->NppaymentRows->find();
            $totalCrCase = $NppaymentRow->newExpr()
                ->addCase(
                    $NppaymentRow->newExpr()->add(['cr_dr' => 'Cr']),
                    $NppaymentRow->newExpr()->add(['amount']),
                    'integer'
                );
            $totalDrCase = $NppaymentRow->newExpr()
                ->addCase(
                    $NppaymentRow->newExpr()->add(['cr_dr' => 'Dr']),
                    $NppaymentRow->newExpr()->add(['amount']),
                    'integer'
                );
            return $NppaymentRow->select([
                    'total_cr' => $NppaymentRow->func()->sum($totalCrCase),
                    'total_dr' => $NppaymentRow->func()->sum($totalDrCase)
                ])
                ->group('nppayment_id')
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Nppayments->save($nppayment)) {
                ...
                foreach($nppayment->nppayment_rows as $nppayment_row){
                    ...
                    $ledger->voucher_source = 'N P Payment Voucher';
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            //pr($nppayment); exit;
            if ($this->Nppayments->save($nppayment)) {
                $this->Nppayments->Ledgers->deleteAll(['voucher_id' => $nppayment->id, 'voucher_source' => 'N P Payment Voucher']);
                $total_cr=0; $total_dr=0;
                foreach($nppayment->nppayment_rows as $nppayment_row){
                    $ledger = $this->Nppayments->Ledgers->newEntity();
                    $ledger->company_id=$st_company_id;
    ...
    function deleteAllRefNumbers($old_received_from_id,$receipt_id){
        ...
        foreach($ReferenceDetails as $ReferenceDetail){
            if($ReferenceDetail->reference_type=="New Reference" || $ReferenceDetail->reference_type=="Advance Reference"){
                $this->Nppayments->ReferenceBalances->deleteAll(['ledger_account_id' => $ReferenceDetail->ledger_account_id, 'reference_no' => $ReferenceDetail->reference_no]);
    ...
    function deleteOneRefNumbers(){
        ...
        if($old_ref_type=="New Reference" || $old_ref_type=="Advance Reference"){
            ...
            $ReferenceDetail=$this->Nppayments->ReferenceDetails->find()->where(['ledger_account_id'=>$old_received_from_id,'payment_id'=>$payment_id,'reference_no'=>$old_ref])->first();
