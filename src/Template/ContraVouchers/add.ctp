...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption" >
            ...
            <span class="caption-subject font-blue-steel uppercase">Add Contra Vouchers</span>
        </div>
    </div>
    ...
    <?= $this->Form->create($contraVoucher,['id'=>'form_sample_3']) ?>
        ...
        <table width="100%" id="main_table">
            ...
            <tfoot>
                ...
                <td></td>
                <td><button type="submit" class="btn btn-primary" >CREATE CONTRA VOUCHER</button></td>
                <td></td>
            </tfoot>
        </table>
        </div>
...
$(document).ready(function() {
    ...
    function rename_rows(){
        ...
        $("#main_table tbody#main_tbody tr.main_tr").each(function(){
            $(this).find("td:eq(0) select.received_from").select2().attr({name:"contra_voucher_rows["+i+"][received_from_id]", id:"quotation_rows-"+i+"-received_from_id"}).rules('add', {
                        required: true,
            ...
            $(this).find("td:eq(1) input").attr({name:"contra_voucher_rows["+i+"][amount]", id:"quotation_rows-"+i+"-amount"}).rules('add', {
                        required: true,
                        min: 0.01,
                    });
            $(this).find("td:eq(1) select").attr({name:"contra_voucher_rows["+i+"][cr_dr]", id:"quotation_rows-"+i+"-cr_dr"});
            $(this).find("td:nth-child(4) textarea").attr({name:"contra_voucher_rows["+i+"][narration]", id:"quotation_rows-"+i+"-narration"}).rules("add", "required");
            i++;
        });
    }
